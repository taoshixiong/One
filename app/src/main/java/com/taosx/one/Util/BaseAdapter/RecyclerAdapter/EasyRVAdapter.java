package com.taosx.one.Util.BaseAdapter.RecyclerAdapter;
...
import com.taosx.one.Util.BaseAdapter.RecyclerAdapter.helper.DataHelper;
...
public abstract class EasyRVAdapter<T> extends RecyclerView.Adapter<EasyRVHolder> implements DataHelper<T> {
    ...
    public void onBindViewHolder(EasyRVHolder holder, int position) {
        ...
        final T item = mList.get(RealPosition);

        holder.setOnItemViewClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if(ic!=null){
                    ic.onItemClick(v, v.getId(),RealPosition,item);
                }
                if(oic!=null){
                    oic.onItemClick(v, v.getId(),RealPosition,item);
                }
            }
        });
        holder.setOnItemViewLongClickListener(new View.OnLongClickListener() {
            ...
            public boolean onLongClick(View v) {
                if(ic!=null){
                    ic.onItemLongClick(v, v.getId(),RealPosition,item);
                }
                if(oil!=null){
                    oil.onItemLongClick(v, v.getId(),RealPosition,item);
                }
                return false;
            }
        });

        onBindData(holder, RealPosition, item);
    }
